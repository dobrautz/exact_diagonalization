function hubbard1DloopRelease
%hubbard1DloopRelease function version of Hubbard_ED script for compiling
% release for loop over number of sites and different filling factors
...
vecParamU = inputData(1):inputData(2);     % on-site-repulsion parameter U
vecParamT = inputData(3):inputData(4);     % hopping parameter t
latticeDimension = inputData(5);                 % lattice Dimension
nLanczosEigenvalues = inputData(11);       % lanczos eigenvalues
lanczosPrecision = inputData(12);          % lanczos genauigkeit
lanczosSteps = inputData(13);              % max. lanczos schritte
flagApplyPointGroup = inputData(14);       % flag for point group symmetry
flagDoNoSymmetry = inputData(15);          % flag for no symmetry calculation
flagDoOnlyTrans = inputData(16);           % flag for only translation symmetry
flagSaveResults = inputData(17);           % flag for saving
flagLoopOverFilling = inputData(8);        % flag if loop over possible fillings
distHalfFilling = inputData(18);           % constraint for loop over fillings

if latticeDimension == 1
    latticeSizeInput = inputData(6):inputData(7); % lattice parameters
    latticeSaveDir = '1D';
else
    latticeSize = [inputData(6);inputData(7)];
    latticeSaveDir = [num2str(inputData(6)),'X',num2str(inputData(6))];
    latticeSizeInput = 1;
end

for iLatticeSize = 1:numel(latticeSizeInput(1,:))
    
    if latticeDimension == 1
        latticeSize = latticeSizeInput(iLatticeSize);
    end
    % diff. of spins to quarter filling UP
    ...
    if flagLoopOverFilling == 1
        fillingFactor = ceil((-nSites+1+distHalfFilling-oddSiteFactor)/2):...
            floor((nSites-1-distHalfFilling-oddSiteFactor)/2);
    else
        fillingFactor = 0;
    end
    
    
    for iFilling = 1:numel(fillingFactor)
        for jFilling = 1:numel(fillingFactor)
            
            if flagLoopOverFilling == 1
                diffQuarterFillingUp = fillingFactor(iFilling);
                diffQuarterFillingDown = fillingFactor(jFilling);
            else
                diffQuarterFillingUp = inputData(9);
                diffQuarterFillingDown = inputData(10);
            end
            ...
            for iParamU = 1:numel(vecParamU)
                for iParamT = 1:numel(vecParamT)
                    
                    paramU = vecParamU(iParamU);
                    paramT = vecParamT(iParamT);
